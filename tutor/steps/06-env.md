GOAL  受注した案件の環境を確認し、参加者自身が `/start` でアプリを起動し、クライアントから報告された概要ページの不具合を自分の目で確かめる。

SAY
- RFP の要件 R1〜R4 がそのまま SOW になりました。第2幕の時間は約50分なので、今日は、クライアントから報告のあった **概要ページの不具合** を直し、コードレビューまでして納品します。残りは次のフェーズです。
...
4. 起動したら http://localhost:3000 を開いてもらう。リモート環境なら `http://<外部IP>:3000`（公開が必要なのは 3000 だけ。8001 は Vite のプロキシ経由）。画面をあちこちクリックしてもらったうえで、クライアントからの報告を伝える: **「概要ページで期間や場所を選んでも反映されない」**。参加者自身に、概要ページで期間（例: 3月）や場所（例: 東京）を切り替えて確かめてもらい、「何が変わって、何が変わらない？」と聞く。売上は変わるのに、上部の KPI の多く（在庫回転率・出荷完了・注文充足率・平均処理時間）が変わらないことに気づけば上出来。気づかなければ、KPI の数字を指して一緒に見る。
