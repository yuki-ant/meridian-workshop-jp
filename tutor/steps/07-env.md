GOAL  受注した案件の環境を確認し、参加者自身が `/start` でアプリを起動し、Reports ページの異常に気づく。

SAY
- RFP の要件 R1〜R4 がそのまま SOW になりました。第2幕の時間は約40分なので、今日は **R1（Reports の改修）のうち最も効く一点** を納品します。残りは次のフェーズです。
...
4. 起動したら http://localhost:3000 を開いてもらう。リモート環境なら `http://<外部IP>:3000`（公開が必要なのは 3000 だけ。8001 は Vite のプロキシ経由）。画面をあちこちクリックしてもらい、「Reports ページで気になるところは？」と聞く。フィルターを変えても Reports の数字が変わらないことに気づけば上出来。気づかなければ、そこを一緒に見る。

RECORD  reports_issue=<参加者が気づいたことを短く>

DONE WHEN  8001 と 3000 が応答し、参加者が Reports の異常を一つ挙げた。
