/* slides.jsx — the pages of the introduction deck (ten, plus a speaker page when presenter.js names a presenter). Layout is sized off the 1280×720 canvas with flex and %,
   never a literal canvas width. Copy stays short: what a presenter would say lives in each slide's `notes`.
   The deck must not give away what the learner is meant to find out (the gaps in the RFP, how thin the hand-off
   memo is, which KPI tiles ignore the filters). */
...
function Slide_Title() {
  const dot = (c, k) => <span key={k} style={{ width: 9, height: 9, borderRadius: 5, background: c, display: 'inline-block' }} />;
  const lbl = { font: `600 11.5px/1 ${MONO}`, letterSpacing: '0.1em', color: 'rgba(250,249,245,0.62)', whiteSpace: 'nowrap' };
  return (
    <Slide bg={C.SLATE} color={C.IVORY} label="Title" padding="0 112px">
      <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', height: '100%' }}>
        ...
        <div style={{ fontSize: 18.5, color: 'rgba(250,249,245,0.76)', marginTop: 34, lineHeight: 1.75 }}>
          Meridian Components の RFP に応札し、最初の改修を納品する · {nb('約 90 分')}<br />
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 44 }}>
          ...
          <span style={{ width: 11, height: 13, borderRadius: 2, border: '2px solid #9CC3B5', display: 'inline-block' }} />
...
/* ── Speaker — only when tutor/pages/intro/presenter.js sets window.PRESENTER ─ */
    ...
    <Slide label="Speaker" padding="44px 72px 60px">
      <div style={{ display: 'flex', gap: 56, flex: 1, minHeight: 0, alignItems: 'center' }}>
        <div style={{ flex: 0.82, minWidth: 0 }}>
          <div style={{ font: `600 12.5px/1.4 ${MONO}`, letterSpacing: '0.14em', color: C.CLAY, marginBottom: 26 }}>SPEAKER INTRODUCTION · 講師紹介</div>
          {P.photo
        ...
        <div style={{ flex: 0.8, display: 'flex', flexDirection: 'column', gap: 12, minWidth: 0 }}>
          <div style={{ flex: 1, background: '#fff', border: `1px solid ${C.LINE}`, borderRadius: 14, overflow: 'hidden', display: 'flex', flexDirection: 'column', minHeight: 0 }}>
            ...
            <div style={{ padding: '12px 18px' }}>
              <div style={{ font: `600 12px/1.4 ${MONO}`, letterSpacing: '0.12em', color: C.CLAY }}>第1幕 · ステップ 02 · 約 4 分</div>
              <div style={{ fontSize: 17.5, fontWeight: 700, lineHeight: 1.4, margin: '5px 0 14px' }}>{ph('RFP とは — 流れと、', '応札側が考えること')}</div>
              <div style={{ position: 'relative', display: 'flex', justifyContent: 'space-between', padding: '0 6px' }}>
                <span style={{ position: 'absolute', left: 14, right: 14, top: 7, height: 2, background: C.OAT }} />
                {flow.map((t, i) => (
                  <span key={t} style={{ position: 'relative', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 5 }}>
                    <span style={{ width: 16, height: 16, borderRadius: 8, background: i === 2 ? C.CLAY : '#fff', border: `2px solid ${i === 2 ? C.CLAY : C.SKY}` }} />
                    <span style={{ fontSize: 12, color: i === 2 ? C.RUST : C.GRAY, fontWeight: i === 2 ? 700 : 400 }}>{t}</span>
                  </span>
                ))}
              </div>
              <div style={{ marginTop: 14, borderTop: `1px solid ${C.LINE}`, paddingTop: 9, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '5px 14px', fontSize: 12.5, color: C.INK }}>
                {['1 · RFI・RFP・RFQ の違い', '2 · RFP の標準的な流れ', '3 · 応札するかどうか', '4 · RFP の読み方：4 つの要点', '5 · 提案書の構成と書き方', '6 · 勝ち筋を決める'].map((t) => <span key={t} style={{ whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{t}</span>)}
              </div>
            </div>
          </div>
          <div style={{ background: '#fff', border: `1.5px solid ${C.MINERAL}`, borderRadius: 12, padding: '10px 14px', fontSize: 13.5, lineHeight: 1.6 }}>
            <b style={{ color: C.GREEN }}>「Done」と言うだけでは進みません。</b>Claude が小さなチェッカーを実行し、ファイル、ブランチ、コミット、起動中のアプリを確かめてから次へ進みます。
          </div>
        </div>
      </div>
      <div style={{ display: 'flex', gap: 14, marginTop: 12, paddingTop: 11, borderTop: `1px solid ${C.LINE}` }}>
        {beat(1, 'Claude が説明する', '目安の時間も伝える', C.SLATE)}
        {beat(2, 'あなたが読む・決める', 'ページは 1 本 4 分ほど', C.SKY)}
        {beat(3, '「Done」と伝える', <span>/start、Shift+Tab、@ は自分で入力</span>, C.CLAY)}
...
function FileIcon({ kind }) {
  const s = { width: 26, height: 26, flexShrink: 0 };
  if (kind === 'code') {
    return (
...
function Slide_Case() {
  const req = (id, t, top) => (
    <div key={id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '7px 0', borderBottom: `1px solid ${C.LINE}`, fontSize: 15.5 }}>
      ...
      {top && <span style={{ marginLeft: 'auto', font: `600 11px/1 ${MONO}`, letterSpacing: '0.08em', color: '#fff', background: C.CLAY, borderRadius: 6, padding: '5px 8px', whiteSpace: 'nowrap' }}>最優先</span>}
    </div>
  );
  const day = (d, t, on) => (
  ...
  const file = (kind, path, what) => (
    <div style={{ display: 'flex', gap: 14, alignItems: 'center', padding: '13px 0', borderBottom: `1px solid ${C.LINE}` }}>
      <FileIcon kind={kind} />
      <div style={{ minWidth: 0 }}>
        <div style={{ fontFamily: MONO, fontSize: 15, color: C.SLATE, overflowWrap: 'anywhere' }}>{path}</div>
        <div style={{ fontSize: 15, color: C.GRAY, marginTop: 2 }}>{what}</div>
      </div>
    </div>
  );
  return (
    <Slide label="The case" padding="40px 72px 58px">
      ...
      <div style={{ display: 'flex', gap: 34, flex: 1, minHeight: 0 }}>
        ...
        <div style={{ flex: 0.92, minWidth: 0, display: 'flex', flexDirection: 'column' }}>
          <div style={{ font: `600 12px/1.4 ${MONO}`, letterSpacing: '0.12em', color: C.GRAY, textTransform: 'uppercase' }}>RFP パッケージに入っているもの</div>
          {file('doc', 'docs/rfp/MC-2026-0417.md', 'RFP の本体')}
          {file('note', 'docs/rfp/meridian-background.md', 'クライアントの背景資料（社内のリサーチノート）')}
          {file('memo', 'docs/rfp/vendor-handoff.md', '前任ベンダーの引き継ぎメモ')}
          {file('code', 'client/ · server/', '前任ベンダーが残したソースコード')}
          <div style={{ marginTop: 'auto', background: C.SLATE, color: C.IVORY, borderRadius: 12, padding: '13px 18px' }}>
            <div style={{ fontSize: 18, fontWeight: 700, lineHeight: 1.4, marginBottom: 4 }}>エンジニアでなくても進められます</div>
            <div style={{ fontSize: 14, color: 'rgba(250,249,245,0.82)', lineHeight: 1.6 }}>決めるのはあなた、提案書の作文とコードは Claude。Claude は試験官ではなく、あなたのペアです。</div>
          </div>
        </div>
      </div>
    </Slide>
...
/* ── 4 · The app and R1 — drawn in the app's own light palette ─────────────── */
...
function AppMock({ phase }) {
  const ring = { outline: `2.5px solid ${C.CLAY}`, outlineOffset: 2, borderRadius: 7 };
  const sel = (label, value, on) => (
    <div style={{ flex: 1, minWidth: 0 }}>
      <div style={{ fontSize: 10.5, color: APP.sub, marginBottom: 3 }}>{label}</div>
      <div style={Object.assign({ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#fff', border: `1px solid ${on ? APP.blue : '#cbd5e1'}`, borderRadius: 6, padding: '5px 8px', fontSize: 12, color: APP.ink, fontWeight: on ? 700 : 400, transition: 'all 0.3s ease' }, on ? ring : {})}>
        <span style={{ whiteSpace: 'nowrap', overflow: 'hidden' }}>{value}</span><span style={{ color: APP.sub, fontSize: 9 }}>▼</span>
      </div>
    </div>
  );
  const kpi = (label, value, goal, w, ok) => (
    <div style={{ flex: 1, minWidth: 0, background: APP.card, border: `1px solid ${APP.line}`, borderRadius: 9, padding: '9px 10px' }}>
      <div style={{ fontSize: 10.5, color: APP.sub, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{label}</div>
      <div style={{ fontSize: 21, fontWeight: 700, color: APP.ink, margin: '2px 0 1px', whiteSpace: 'nowrap' }}>{value}</div>
      <div style={{ fontSize: 9.5, color: APP.sub, whiteSpace: 'nowrap', overflow: 'hidden' }}>{goal}</div>
  ...
  return (
    <div style={{ background: APP.bg, borderRadius: 14, overflow: 'hidden', boxShadow: '0 12px 34px rgba(20,20,19,0.18)', border: `1px solid ${C.LINE}`, color: APP.ink, display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div style={{ position: 'relative', background: '#fff', borderBottom: `1px solid ${APP.line}`, padding: '9px 16px 11px', display: 'flex', gap: 10 }}>
        ...
        {sel('期間', phase >= 1 ? '3月' : 'すべての月', phase === 1)}
        {sel('場所', phase >= 2 ? '東京' : 'すべて', phase === 2)}
        {sel('カテゴリ', 'すべて')}
        {sel('注文ステータス', 'すべて')}
      </div>
      <div style={{ padding: '12px 16px', flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ position: 'relative', borderRadius: 10, padding: 8, margin: -8, outline: phase >= 3 ? `2.5px dashed ${C.CLAY}` : '2.5px dashed transparent', transition: 'outline-color 0.4s ease' }}>
          <Pin n={2} style={{ position: 'absolute', left: -6, top: -10, zIndex: 2 }} />
          <div style={{ fontSize: 11.5, fontWeight: 700, color: APP.sub, margin: '0 0 6px 22px' }}>主要業績評価指標</div>
          <div style={{ display: 'flex', gap: 8 }}>
            {kpi('在庫回転率', '4.2', '目標: 4.5 (-6.67%)', '93%')}
            {kpi('注文履行数', '187', '目標: 200 (93.50%)', '93%')}
            {kpi('注文充足率', '96.8%', '目標: 95% (+1.80%)', '100%', true)}
            {kpi('平均処理時間（日）', '2.8', '目標: 3.0 (-6.67%)', '93%', true)}
          </div>
          <div style={{ position: 'absolute', right: 10, top: -13, background: C.CLAY, color: '#fff', font: `700 12px/1 ${SANS}`, padding: '6px 10px', borderRadius: 6, opacity: phase >= 3 ? 1 : 0, transition: 'opacity 0.4s ease', whiteSpace: 'nowrap' }}>期間や場所を選んでも、更新されない？</div>
        </div>
        <div style={{ position: 'relative', flex: 1, minHeight: 0, marginTop: 18, display: 'flex', gap: 10, overflow: 'hidden' }}>
...
function Slide_App() {
  const phase = useLoop(4, 1500, 3);
  const row = (k, v) => (
    <div style={{ display: 'flex', gap: 12, padding: '8px 0', borderBottom: `1px solid ${C.LINE}`, alignItems: 'baseline' }}>
      <span style={{ font: `12px/1.4 ${MONO}`, letterSpacing: '0.06em', color: C.GRAY, width: 50, flexShrink: 0 }}>{k}</span>
      <span style={{ fontSize: 14.5, lineHeight: 1.6 }}>{v}</span>
    </div>
  );
  const stepChip = (t, i) => (
    <span key={i} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 13.5, whiteSpace: 'nowrap' }}>
      {i > 0 && <span style={{ color: C.CLAY }}>→</span>}<span style={{ background: '#fff', border: `1px solid ${C.LINE}`, borderRadius: 7, padding: '4px 8px' }}>{t}</span>
    </span>
  );
  return (
    <Slide label="The app and R1" padding="40px 72px 58px">
      <Head kicker="第2幕の題材" h2={ph('最初の納品は R1：', '概要ページの指標が、', 'フィルターに連動しない')} />
      <div style={{ display: 'flex', gap: 32, flex: 1, minHeight: 0 }}>
        ...
        <div style={{ flex: 0.88, minWidth: 0, display: 'flex', flexDirection: 'column' }}>
          <div style={{ background: '#fff', borderLeft: `4px solid ${C.CLAY}`, borderRadius: '0 12px 12px 0', padding: '12px 16px', boxShadow: '0 1px 0 rgba(31,30,29,0.06)' }}>
            <div style={{ fontSize: 15.5, lineHeight: 1.65, fontWeight: 500 }}>「概要ページの主要指標（KPI）が、画面上部の期間・拠点などのフィルターを変えても更新されない。」</div>
            <div style={{ font: `12px/1.4 ${MONO}`, color: C.GRAY, marginTop: 5 }}>RFP §3.1 · R1（クライアントの最優先）</div>
          </div>
          <div style={{ fontSize: 14, color: C.INK, lineHeight: 1.65, margin: '10px 0 4px' }}>オペレーションチームは数字を信用できず、表計算ソフトで集計し直しています。</div>
          {row('構成', <span>Vue 3 + Vite（ポート 3000）と FastAPI（ポート 8001）。データは JSON ファイルで、DB はありません。</span>)}
          {row('確認', <span>{nb(<span><Pin n={1} size={18} style={{ verticalAlign: '-3px', boxShadow: 'none' }} /> を切り替えて、</span>)}{nb(<span><Pin n={2} size={18} style={{ verticalAlign: '-3px', boxShadow: 'none' }} /> の何が変わり、</span>)}何が変わらないか。自分の目で確かめるところから始めます。</span>)}
          <div style={{ marginTop: 12, display: 'flex', flexWrap: 'wrap', gap: '7px 6px' }}>
            {['/start で起動', '症状を確かめる', 'Plan Mode で原因を調べる', '直してコミット', 'サブエージェントがレビュー'].map(stepChip)}
          </div>
          <div style={{ marginTop: 'auto', background: C.SLATE, color: C.IVORY, borderRadius: 12, padding: '11px 16px', fontSize: 14, lineHeight: 1.6 }}>
...
function Slide_Start() {
  const need = (t, s) => (
    <div style={{ display: 'flex', gap: 10, alignItems: 'baseline', padding: '14px 0', borderBottom: `1px solid ${C.LINE}` }}>
      <b style={{ fontSize: 18, width: 150, flexShrink: 0 }}>{t}</b><span style={{ fontSize: 15.5, color: C.GRAY, lineHeight: 1.5 }}>{s}</span>
    </div>
  );
  const ln = (n, body) => <div><span style={{ color: C.CLAY }}>{n}</span>&nbsp;&nbsp;{body}</div>;
  return (
    <Slide label="Start" padding="40px 72px 58px">
      <div style={{ position: 'absolute', right: 72, top: 46, fontSize: 13, color: C.GRAY, background: C.TINT, borderRadius: 8, padding: '6px 12px' }}>すでに Claude が起動している方は、次のページへ</div>
      <Head kicker="始め方" h2={ph('fork して clone し、', 'コマンドを 1 つ実行する')} />
      <div style={{ display: 'flex', gap: 36, flex: 1, minHeight: 0, alignItems: 'stretch' }}>
        <div style={{ flex: 1.12, display: 'flex', flexDirection: 'column', gap: 20, minWidth: 0 }}>
          <div style={{ background: C.SLATE, borderRadius: 14, padding: '20px 26px', fontFamily: MONO, fontSize: 17, lineHeight: 2.35, whiteSpace: 'nowrap', overflow: 'hidden', color: C.IVORY, boxShadow: '0 10px 30px rgba(0,0,0,0.18)' }}>
            {ln(1, <span><span style={{ color: 'rgba(250,249,245,0.55)' }}># GitHub でこのリポジトリを fork してから</span></span>)}
            ...
            {ln(4, <span><span style={{ color: 'rgba(250,249,245,0.6)' }}>&gt;</span> こんにちは</span>)}
          </div>
          <div style={{ fontSize: 16.5, color: C.GRAY, lineHeight: 1.85 }}>
            {code('start.sh', 15)} は {code('claude --plugin-dir tutor', 15)} を実行するだけです。Windows では {code('start.cmd', 15)}。fork しておくと、最後に Pull Request を作れます。
          </div>
          <div style={{ marginTop: 'auto', background: '#fff', border: `1.5px solid ${C.CLAY}`, borderRadius: 12, padding: '18px 24px' }}>
            <div style={{ fontSize: 26, fontWeight: 700, marginBottom: 6 }}>何もインストールされません。</div>
            <div style={{ fontSize: 16, color: C.INK, lineHeight: 1.7 }}>チューターが読み込まれるのは、このセッションだけです。ほかの場所で普通に {code('claude', 14)} を起動したときの動きは、これまでと変わりません。</div>
          </div>
        </div>
        <div style={{ flex: 0.88, minWidth: 0, display: 'flex', flexDirection: 'column' }}>
          <div style={{ font: `600 12px/1.4 ${MONO}`, letterSpacing: '0.12em', color: C.GRAY, textTransform: 'uppercase', marginBottom: 2 }}>必要なもの</div>
          {need('Claude Code', 'インストール済み、サインイン済み')}
          {need('Node.js 18 以上', 'チューターとフロントエンドが使用')}
          {need('uv', '第2幕でバックエンドの起動に使用。なければ Claude が案内')}
          {need('git', 'fork を clone し、作業をコミット')}
          {need('ブラウザ', '「いまどこ？」ページと、読み物を表示')}
          <div style={{ marginTop: 'auto', fontSize: 16, color: C.INK, lineHeight: 1.7 }}>
            <b>いつ中断しても構いません。</b>{code('./start.sh')} をもう一度実行すれば、Claude が同じステップから再開します。進み具合は {code('tutor/state/')} に保存されています。
          </div>
        </div>
      </div>
    </Slide>
...
/* ── 6 · Agenda: one bar per step, sized by its planned minutes ────────────── */
const STEPS_P1 = [
  ...
  ['04', 'RFP の穴を探す', 6],
  ['05', '提案の方針を決める · 提案書は Claude が書く', 8, '選択式の質問に答える'],
  ['06', 'HTML ページとスライドにする', 5, '見せる形を生成'],
];
const STEPS_P2 = [
  ['07', '受注 · アプリを起動して症状を確かめる', 6, '/start（カスタムコマンド）'],
  ['08', 'R1 の原因を調べて計画する', 6, 'Plan Mode（Shift+Tab）'],
  ['09', 'R1 を直し、ブラウザで確かめてコミット', 22, '実装 → 確認 → コミット'],
  ['10', 'コードレビュー · 直す指摘を選ぶ', 10, 'サブエージェント'],
  ['11', 'まとめ · 進捗レポート', 7, 'レポートを自動作成'],
];
function Slide_Agenda() {
  const rowOf = (color) => ([n, t, m, chip]) => (
    <div key={n}>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, fontSize: 15.5 }}>
        ...
        {chip && <span style={{ marginLeft: 'auto', fontSize: 11.5, padding: '3px 7px', borderRadius: 6, background: '#D9775722', border: '1px solid #D9775755', color: C.RUST, whiteSpace: 'nowrap', flexShrink: 0 }}>{chip}</span>}
      </div>
  ...
  return (
    <Slide label="Agenda" padding="38px 72px 56px">
      ...
      <div style={{ marginTop: 10, fontSize: 13.5, color: C.GRAY }}>
        合計 {nb('約 85 分')}、予備 {nb('約 5 分')}、予算は {nb('90 分')}。<span style={{ color: C.RUST }}>色付きのラベル</span> = そのステップで初めて使う Claude Code の機能。必要になったときに、Claude が紹介します。
      </div>
    </Slide>
  );
}

/* ── 7, 8 · Real screens from an example run ───────────────────────────────── */
function Shot({ src, url, alt, flex, size, pins, bg }) {
  // The image always fills the frame's width; a tall page simply continues below the frame. `size` is the image's own
  // width / height: with it, `pins` (x and y in percent of the whole image) stay on their spot at any frame size.
  return (
    <div style={{ flex: flex || 1, minHeight: 0, minWidth: 0, display: 'flex', flexDirection: 'column', background: bg || '#fff', border: `1px solid ${C.LINE}`, borderRadius: 12, overflow: 'hidden', boxShadow: '0 10px 28px rgba(20,20,19,0.10)' }}>
...
function Callout({ n, c, title, children }) {
  return (
    <div style={{ flex: 1, minWidth: 0, display: 'flex', gap: 10, alignItems: 'flex-start' }}>
      <span style={{ position: 'relative', top: 1, display: 'inline-flex' }}><Pin n={n} c={c} /></span>
      <div style={{ minWidth: 0 }}>
        <div style={{ fontSize: 16, fontWeight: 700, lineHeight: 1.35, color: C.SLATE }}>{title}</div>
        <div style={{ fontSize: 13, lineHeight: 1.55, color: C.INK, marginTop: 2 }}>{children}</div>
      </div>
    </div>
  );
}
function Slide_Map() {
  return (
    <Slide label="Live map" padding="40px 72px 56px">
      <Head kicker="進行中" h2={ph('「いまどこ？」ページが、', '現在地と残り時間を示し続ける')} />
      <div style={{ display: 'flex', gap: 20, flex: 1, minHeight: 0 }}>
        <Shot src="assets/map-top.png" size={1400 / 1036} url="localhost:8766/map.html · このタブは開いたままに（画面は表示例）" alt="ステップ 09 の時点の「いまどこ？」ページの上半分。現在のステップ、経過時間、予定終了、ペース、90 分の予算の帯、いまやること、あなたの成果物" pins={[{ n: 1, x: 84, y: 15.8 }, { n: 2, x: 63, y: 38 }, { n: 3, x: 91, y: 51.5 }]} />
        <Shot src="assets/map-steps.png" size={1400 / 973} url="同じページの下半分 · 全体マップ" alt="全体マップ。12 のステップが二幕に分かれて並び、終わったステップにはチェックと完了時刻が付き、いまのステップは黒い帯で示される" pins={[{ n: 4, x: 52.5, y: 39 }]} />
      </div>
      <div style={{ display: 'flex', gap: 18, marginTop: 14, marginRight: 120 }}>
        <Callout n={1} title="現在地が 1 行でわかる">いまのステップと、完了した数。</Callout>
        <Callout n={2} title="90 分の予算とペース">帯が経過時間、縦線が予定。遅れたら Claude が次を短めに運びます。</Callout>
        <Callout n={3} title="いまやること">次の一手と、あなたの成果物へのリンク。</Callout>
...
function Slide_Report() {
  const G = C.GREEN;
  const chip = (t) => <span key={t} style={{ fontSize: 12, padding: '3px 7px', borderRadius: 6, background: '#fff', border: `1px solid ${C.LINE}`, color: C.INK, whiteSpace: 'nowrap' }}>{t}</span>;
  return (
    <Slide label="Progress report" padding="40px 72px 56px">
      <Head kicker="最後に" h2={ph('今日の進捗を、', 'レポートに自動でまとめる')} />
      <div style={{ display: 'flex', gap: 20, flex: 1, minHeight: 0 }}>
        <Shot src="assets/report-top.png" size={1400 / 1190} url="localhost:8766/proposal/progress-report.html（画面は表示例）" alt="進捗レポートの冒頭。今日の見出しと要約、日付、案件、完了したステップ、かかった時間。その下に概要の 4 つの数字" pins={[{ n: 1, x: 95, y: 16.5, c: G }, { n: 2, x: 93, y: 68.5, c: G }]} />
        <Shot src="assets/report-timeline.png" size={1400 / 1008} url="同じレポートの、少し下 · タイムライン" alt="レポートのタイムライン。1 ステップにつき 1 行で、目安の時間の帯と、実際にかかった時間の帯が並ぶ" pins={[{ n: 3, x: 66, y: 42, c: G }]} />
      </div>
      <div style={{ display: 'flex', gap: 18, marginTop: 14, marginRight: 120 }}>
        <Callout n={1} c={G} title="今日 1 日を、1 行で">文章は Claude が書き、数字はあなたの実行記録から取ります。</Callout>
        <Callout n={2} c={G} title="概要の数字">成果物、納品したコミット、テスト、かかった時間。</Callout>
        <Callout n={3} c={G} title="予定と実績">{nb('1 ステップ')}につき {nb('1 行')}。目安と、実際の時間。</Callout>
        <div style={{ flex: 1.15, minWidth: 0 }}>
          <div style={{ font: `600 12px/1.4 ${MONO}`, letterSpacing: '0.1em', color: C.GRAY, marginBottom: 5 }}>さらに下</div>
          <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap' }}>{['提案書', '納品', 'コードレビュー', '判断と軌道修正', '次フェーズへの引き継ぎ', '使った機能'].map(chip)}</div>
        </div>
      </div>
    </Slide>
  );
...
/* ── 9 · What you keep ─────────────────────────────────────────────────────── */
function Slide_Keep() {
  const file = (path, what, c) => (
    <div style={{ background: 'rgba(250,249,245,0.06)', border: '1px solid rgba(250,249,245,0.16)', borderRadius: 14, padding: '18px 26px', minWidth: 0, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
      <div style={{ fontFamily: MONO, fontSize: 16.5, color: c || C.CLAY, marginBottom: 10, overflowWrap: 'anywhere' }}>{path}</div>
      <div style={{ fontSize: 17, color: 'rgba(250,249,245,0.85)', lineHeight: 1.7 }}>{what}</div>
    </div>
  );
  return (
    <Slide bg={C.SLATE} color={C.IVORY} label="What you keep" padding="42px 72px 60px">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gridAutoRows: '1fr', gap: 14, flex: 1, minHeight: 0 }}>
        {file('proposal/proposal.md', <span>あなたの{nb(' 4 つ')}の判断をもとに、Claude が書き上げた提案書。置いた前提条件も明記されています。</span>)}
        {file(<span>proposal/proposal.html<br />proposal/capabilities-deck.html</span>, '同じ提案書から作った、読む用のページと、プレゼン用のスライド。')}
        {file('git log · fix/overview-filters', '概要ページの KPI をフィルターに連動させたコミットと、レビューの指摘を直したコミット。', '#9CC3B5')}
        {file('コードレビューの記録', 'サブエージェントの指摘と、直すか次フェーズに回すかの判断。')}
        {file('proposal/progress-report.html', <span>今日の成果、予定と実績、次フェーズへの引き継ぎを{nb(' 1 ページ')}に。</span>, '#9CC3B5')}
        {file('fork → Pull Request', '自分の fork に push して PR を作るところまで、Claude が手伝います。')}
      </div>
    </Slide>
  );
}

/* ── 10 · Help, and back to the terminal ───────────────────────────────────── */
function Slide_Help() {
  const term = (lines) => (
    <div style={{ marginTop: 'auto', background: C.SLATE, borderRadius: 10, padding: '12px 14px', fontFamily: MONO, fontSize: 14, lineHeight: 1.75, color: 'rgba(250,249,245,0.9)' }}>
      {lines.map(([who, t], i) => (
  ...
  const card = (k, t, body, lines) => (
    <div style={{ flex: 1, background: '#fff', border: `1px solid ${C.LINE}`, borderRadius: 14, padding: '20px 24px', minWidth: 0, display: 'flex', flexDirection: 'column', gap: 0 }}>
      <div style={{ font: `600 13.5px/1.4 ${MONO}`, letterSpacing: '0.12em', color: C.CLAY }}>{k}</div>
      <div style={{ fontSize: 26, fontWeight: 700, lineHeight: 1.4, margin: '8px 0 10px' }}>{t}</div>
      <div style={{ fontSize: 17, color: C.INK, lineHeight: 1.75, marginBottom: 12 }}>{body}</div>
      {term(lines)}
    </div>
  );
  return (
    <Slide label="Help" padding="42px 72px 58px">
      ...
      <div style={{ display: 'flex', gap: 16, flex: 1, alignItems: 'stretch', minHeight: 0 }}>
        {card('行き詰まったら', ph('Claude に', 'そう伝えます。'), <span>足りないものを一緒に片付けるか、時間がなければ、確認のうえで先へ進めます。質問はいつでも構いません。</span>, [['you', 'アプリが起動しません'], ['claude', 'uv が見つからないようです。入れ方を案内しますね。']])}
        {card('中断するとき', ph('ランチャーを', '再実行します。'), <span>いつ終了しても構いません。同じステップから、新しいセッションが始まります。</span>, [['sh', './start.sh'], ['you', 'こんにちは'], ['claude', 'おかえりなさい。ステップ 08 の続きから始めます。']])}
        {card('やり直すとき', ph('スクリプト 1 つで', '戻せます。'), <span>進捗を消し、リポジトリを出発点に戻します。それまでの作業は、退避用のブランチに残ります。</span>, [['sh', './reset-demo.sh'], ['out', 'これまでの作業はブランチ tutor-backup-20260923-141205 に残しました。'], ['out', 'リセットしました。./start.sh でもう一度始められます。']])}
      </div>
      <div style={{ marginTop: 16, marginRight: 150, background: C.CLAY, color: '#fff', borderRadius: 12, padding: '14px 22px', display: 'flex', alignItems: 'center', gap: 18, boxShadow: '0 6px 24px rgba(0,0,0,0.15)' }}>
        <span style={{ fontSize: 21, fontWeight: 700, whiteSpace: 'nowrap' }}>では、ターミナルへ。</span>
        <span style={{ fontSize: 15, lineHeight: 1.6 }}>Claude が待っている方は、戻って <b style={{ fontFamily: MONO }}>Done</b> と送ってください。これから始める方は <b style={{ fontFamily: MONO }}>./start.sh</b> を実行します。</span>
      </div>
    </Slide>
  );
}
...
const SLIDES = [
  { c: Slide_Title, title: 'Claude が Claude を教える', notes: 'パートナー向けの、セルフペース型の Claude ハンズオンワークショップです。講師はいません。Claude Code 自身が、あなたのターミナルの中で、約 90 分かけて 1 ステップずつ案内します。題材は、Meridian Components という架空のクライアントの RFP です。第1幕でコンサルタントとして応札し、受注した第2幕で、最初の改修を納品します。' },
  window.PRESENTER && window.PRESENTER.name && { c: Slide_Speaker, title: '講師紹介', notes: '講師の自己紹介です。名前、所属、これまでの経歴を、ここで短く話してください。このページは tutor/pages/intro/presenter.js の内容から作られます。講師が変わるときは、そのファイルを書き換えるだけで、ビルドは要りません。ファイルを空にすると、このページは表示されません。' },
  { c: Slide_How, title: 'Claude が 1 ステップずつ案内し、あなたが判断して手を動かす', notes: '進み方は、いつも同じです。Claude が次にやることと目安の時間を伝えます。短い読み物のページを案内することもあります。読み終えたり、作業が終わったりしたら、ターミナルに戻って「Done」と送ります。「できました」でも構いません。スラッシュコマンド、Shift+Tab、@ によるファイル参照、許可の確認は、あなたにしか入力できないので、Claude は何を入力するかを伝えて待ちます。そのあと、小さなチェッカーがリポジトリの状態と記録を見て、そのステップの完了条件が満たされているかを確かめます。[ターミナル、ページ、チェックの行の順に指す]' },
  { c: Slide_Case, title: 'あなたはコンサルタント。Meridian Components から RFP が届いた', notes: 'あなたの役は、RFP に応札するコンサルタントです。クライアントは Meridian Components。産業用オートメーション部品のディストリビューターで、倉庫はサンフランシスコ、ロンドン、東京にあります。依頼は、在庫管理ダッシュボードのモダナイズと機能拡張です。必須項目は R1 から R5 までで、優先順位順に並んでいます。最優先の R1 が、今日の第2幕で納品するものです。パッケージには、RFP の本体、社内でまとめた背景資料、前任ベンダーの引き継ぎメモ、そしてソースコードが入っています。エンジニアでなくても進められます。決めるのはあなたで、作文とコードは Claude が受け持ちます。' },
  { c: Slide_App, title: '最初の納品は R1：概要ページの指標が、フィルターに連動しない', notes: '第2幕で手を入れるのが、このアプリです。Vue と FastAPI の小さな在庫ダッシュボードで、画面の上に期間や場所のフィルターがあり、その下に主要な指標が並びます。RFP には、フィルターを変えても指標が更新されない、と書かれています。ただし、書かれているのは症状だけです。何が変わって、何が変わらないのかは、アプリを起動して自分の目で確かめます。そのうえで Plan Mode で原因を調べ、直してコミットし、サブエージェントにレビューさせます。R2 から R5 は次のフェーズで、最後のレポートに引き継ぎとしてまとめます。[1 番のフィルター、2 番の指標の順に指す]' },
  { c: Slide_Start, title: 'fork して clone し、コマンドを 1 つ実行する', notes: '始め方です。最後に Pull Request を作れるように、最初にリポジトリを fork して、自分の fork を clone します。あとは start.sh を実行して、Claude に「こんにちは」と声をかけるだけです。ランチャーは、このセッションにだけチューターのプラグインを読み込みます。何もインストールされません。必要なのは、Claude Code、Node.js 18 以上、uv、git、ブラウザです。uv は第2幕まで使わないので、なければその時点で Claude が導入を案内します。いつ中断しても構いません。もう一度 start.sh を実行すれば、同じステップから再開します。' },
  { c: Slide_Agenda, title: '応札する第1幕、納品する第2幕、最後に進捗レポート', notes: '全体は 12 ステップです。第1幕は約 34 分。RFP とは何かを読み、実際の RFP を @ で読み込み、決めないと見積もれない点を探し、提案の方針を 4 つ決めます。提案書の作文は Claude が一気にやり、HTML のページとスライドにもします。第2幕は約 51 分。/start でアプリを起動して症状を確かめ、Plan Mode で原因を調べ、直してコミットし、サブエージェントのレビューから直す指摘を選びます。いちばん長いのは R1 を直すステップで、22 分です。最後に Claude が進捗レポートを作ります。合計は約 85 分で、5 分ほどの予備があります。' },
  { c: Slide_Map, title: '「いまどこ？」ページが、現在地と残り時間を示し続ける', notes: '進行中の様子です。最初のステップで Claude がこのページをブラウザで開き、あとは自動で更新し続けます。タブは開いたままにしておいてください。1 番、いまのステップと完了した数。2 番、90 分の予算に対する経過時間と、予定どおりならどこにいるはずかを示す縦線。3 番、いまやることと、その下にあなたの成果物へのリンク。4 番、右の画面が全体マップです。チェックは「Done」と言った時点ではなく、Claude が完了を確かめた時点で付きます。画面は、サンプルのデータで表示した例です。' },
  ...
  { c: Slide_Keep, title: '作ったものは、リポジトリのファイルとコミットとして手元に残る', notes: 'ここで作るものは、どれもリポジトリの中の実際のファイルとコミットです。提案書と、その HTML ページとスライド。概要ページを直したコミット。レビューの記録。進捗レポート。自分の fork に push して Pull Request を作るところまで、Claude が手伝います。' },
  { c: Slide_Help, title: '行き詰まっても、中断しても、続きから再開できる', notes: '行き詰まったら、Claude にそう伝えてください。足りないものを一緒に片付けるか、時間がなければ確認のうえで先へ進めます。中断したときは、start.sh をもう一度実行すれば同じステップに戻れます。最初からやり直すときは reset-demo.sh です。それまでの作業は退避用のブランチに残ります。では、ターミナルに戻りましょう。Claude が待っている方は「Done」と送ってください。これから始める方は start.sh を実行します。' },
].filter(Boolean);
